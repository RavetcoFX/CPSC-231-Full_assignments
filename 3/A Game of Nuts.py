...
def endgame_repeat(Amount_of_Nuts,name = None, Turn_Number=0, Players = 0):
    ...
    if (Amount_of_Nuts <= 0): #If the last nut has been taken
        ...
        if Players == 1: #If you are playing a Human vs Human game; this is the default state
            ...
            if (Turn_Number % 2 == 0):
                print("\nPlayer Two Wins")                           
        if (Players == 2 or Players ==3): #If you are playing against an AI
            if not (Turn_Number % 2 == 0):
                print("\nPlayer One Wins")
            if (Turn_Number % 2 == 0):
                print("\nAi Wins")                                
                name.incrementing_Moves(name.Moves_Made)
...
class Hats_class:
    ...
    def Init_Hats(name,Amount_of_Nuts):
        ...
        for i in range(Amount_of_Nuts):
            Hats += [[5,5,5]]
        return Hats
    def incrementing_Moves(Moves_Made):
        print(Moves_Made)
...
def Probability_Selection(Current_Nuts_Hat,Hats,Amount_of_Nuts,name):
    ...
    if (r_int <= Current_Nuts_Hat[0]):
        ...
        move = 2
    Decrement_Hats(Current_Nuts_Hat, move,Amount_of_Nuts, name,Hats,)
...
def Decrement_Hats(Current_Nuts_Hat, move, Amount_of_nuts,name, Hats = 0, Win = False):
    ...
    if Win ==False:
        if Current_Nuts_Hat[move] > 0:
            ...
            return Hats
    print(Hats)
    Moves_Made = name.Tracking_Moves(None, Amount_of_nuts,move)
...
def AI_Turn(Hats, Amount_of_Nuts, Win, Turn_Number,name, Players = 5, Seen = 1):
    ...
    if Win == False: #Player one will be invalidated because of the while loop but in between the turns, the win variable dosen't get checked unless this if statement is present
        if Seen == 0:
            AI_Move = Probability_Selection(Hats[Amount_of_Nuts-1],Hats,Amount_of_Nuts,name)
...
def Untrrained_AI(Amount_of_Nuts):
    ...
    Win = endgame_repeat(Amount_of_Nuts,Untrained_AI_Hats,Turn_Number, Players)#Initially sets up the Win Variable
    while Win == False:
       #Player Ones Turn
        Win = endgame_repeat(Amount_of_Nuts,Untrained_AI_Hats, Turn_Number, 1)
        Amount_of_Nuts = Player_Turn(Amount_of_Nuts,Turn_Number,Win)
        Turn_Number += 1        
        #AI's Turn
        Amount_of_Nuts = AI_Turn(Hats, Amount_of_Nuts, Win, Turn_Number,Untrained_AI_Hats, Players)
        Win = endgame_repeat(Amount_of_Nuts,Untrained_AI_Hats,Turn_Number, Players)
...
def Trained_AI(Amount_of_Nuts):
    ...
    Turn_Number = 1 #is used to Let you Know what Turn number the Game is on for each Players turn + is used by endgame_repeat to determine who won
    Win = endgame_repeat(Amount_of_Nuts,name,Turn_Number, Players)#Initially sets up the Win Variable
    if Win ==False:#As long as the last nut has not been taken       
        Win = endgame_repeat(Amount_of_Nuts,name,Turn_Number, Players) #Updates Win Variable before Player One's Turn
        while Win == False:
            #Player 1
            Win = endgame_repeat(Amount_of_Nuts,None,Turn_Number, Players)
            Amount_of_Nuts = Player_Turn(Amount_of_Nuts,Turn_Number,Win)
            Turn_Number += 1           
            #AI's Turn
            Amount_of_Nuts = AI_Turn(Hats, Amount_of_Nuts, Win, Turn_Number,name, Players)
            Turn_Number += 1
            if Win == False:
                if Amount_of_Nuts >= 0:
                    print("Amount of nuts remaining %d" %(Amount_of_Nuts))
                if Amount_of_Nuts <= 0:
                    Win = endgame_repeat(Amount_of_Nuts,name,Turn_Number, Players)
