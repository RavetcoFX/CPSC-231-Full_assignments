...
def startpoint(turtles, window_height_width):
    ...
    alex.setpos(0,0) #after spawning alex, move to the center of the  canvas; this will be the starting position for alex
    alex.down()
...
def Fix_Offscreen(name,window_height_width):
    """Function for handling if the turtles move out of the canvas (off screen); the only argument passed is the name of the turtle"""
    max_limit = ((window_height_width//2) - 10) #Uses the windows maximum height and width value -10 so that the turtles cannot move outside the screen
    Negative_Max = -250 #The lowest possible x or y value that is within the canvas
    Negative_Max_New = (random.randint(0,max_limit)) #The coordinate to relocate the turtle to if they go past Negatice_Max value in the x or y
    Positive_Max = 250 #The Highest possible x or y value that is within the canvas
    Positive_Max_new = (random.randint(0,max_limit)) ##The coordinate to relocate the turtle to if they go past the Positive_Max value in the x or y
...
def win_condition(alex, alice, jeff, each_turn_count,window_height_width):
    ...
    while not win: #While the game still hasn't been won each player takes their turn, and jeff presents the games information until the game is won
        ...
        Fix_Offscreen(alex, window_height_width) #checks to make sure Alex is not outside the boundary; if he is then fix it
        Alice_move(alice, alex) #Alice takes her turn
        Fix_Offscreen(alice,window_height_width)#checks to make sure Alice is not outside the boundary; if he is then fix it
...
def Game_loop(turtles,window_height_width):
    ...
    jeff = init_jeff()
    each_turn_count = 0
    win_condition(alex, alice, jeff, each_turn_count,window_height_width)
...
def main():
    ...
    Game_loop(startpoint(use_turtles(init_alex(), init_alice()), window_height_width),window_height_width) #This calls all the functions in the right order to play the game; initializes the turtles and passes them as arguments to use_turtles, which is then brought into startpoint and then brought into the main game loop
